<script lang="ts">
    ...
    import { DalApiError, owners as ownersApi, versions as versionsApi } from '$lib/api';
    ...
    async function toggleYank(version: string, yanked: boolean) {
        ...
        try {
            ...
            fail(err, yanked ? 'Failed to restore version.' : 'Failed to yank version.');
        }
    }
        ...
        <div class="space-y-3">
            {#each versionList as version}
                ...
                >
                    ...
                    <button
                        type="button"
                        disabled={busyAction === `yank:${version.version}` || busyAction === `unyank:${version.version}`}
